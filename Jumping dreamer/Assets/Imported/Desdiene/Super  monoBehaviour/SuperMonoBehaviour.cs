...
namespace Desdiene.Super_monoBehaviour
{
    /*
    ...
    public class SuperMonoBehaviour : MonoBehaviour
    {
        ...
        /// <summary>
        /// Создает объект, взяв того из пула объектов.
        /// С осторожностью использовать в Awake - ObjectPooler может быть не проинициализирован!
        /// </summary>
        /// <typeparam name="T">Тип компонента, который будет получен с игрового объекта после его создания</typeparam>
        /// <param name="prefabKey">Префаб, который будет создан</param>
        /// <param name="position">Позиция создаваемого объекта</param>
        /// <param name="parent">Родитель в иерархии объектов на сцене</param>
        /// <returns>Полученный компонент с игрового объекта</returns>
        public T SpawnFromPoolAndGetComponent<T>(GameObject prefabKey, Vector3 position, Transform parent = null)
        where T : Component
        {
            return ObjectPooler.Instance.SpawnFromPoolAndGetComponent<T>(prefabKey, position, parent);
        }
        ...
        /// <param name="prefabKey">Префаб, который будет создан</param>
        /// <param name="position">Позиция создаваемого объекта</param>
        /// <param name="rotation">Поворот создаваемого объекта</param>
        /// <param name="parent">Родитель в иерархии объектов на сцене</param>
        /// <returns>Полученный компонент с игрового объекта</returns>
        public T SpawnFromPoolAndGetComponent<T>(GameObject prefabKey, Vector3 position, Quaternion rotation, Transform parent = null)
        where T : Component
        {
            return ObjectPooler.Instance.SpawnFromPoolAndGetComponent<T>(prefabKey, position, rotation, parent);
        }
        ...
        public GameObject SpawnFromPool(GameObject prefabKey, Vector3 position, Transform parent = null)
        {
            return ObjectPooler.Instance.SpawnFromPool(prefabKey, position, parent);
        }
